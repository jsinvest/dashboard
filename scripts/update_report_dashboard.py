...
CSS="""
  ...
  .signal-stat-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:8px;margin-bottom:12px}.signal-stat{padding:10px 12px;background:var(--bg3);border:1px solid var(--border);border-radius:7px}.signal-stat small{display:block;color:var(--text-muted);font-size:11px}.signal-stat strong{display:block;margin-top:4px;font-family:'JetBrains Mono',monospace;font-size:18px}.signal-stat.up strong{color:var(--up)}.signal-stat.down strong{color:var(--down)}
...
function renderSignalStats(direction){const rows=reportSignalData[direction]||[],counts=signalCategoryCounts(rows);const box=document.getElementById(direction==='up'?'reportUpStats':'reportDownStats');if(box)box.innerHTML=Object.entries(counts).map(([k,v])=>`<div class="signal-stat ${direction}"><small>${escapeHtml(k)}</small><strong>${v.toLocaleString()}건</strong></div>`).join('');const countEl=document.getElementById(direction==='up'?'reportUpCount':'reportDownCount');if(countEl)countEl.textContent=`총 ${rows.length.toLocaleString()}개 시그널`;}
