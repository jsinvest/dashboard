...
function renderCashIndexReview(data, targetPanel = null) {
  ...
  panel.innerHTML = `<div class="cash-review">
    ...
    <div class="report-meta" id="${embedded ? 'indexContextMeta' : 'supplyMeta'}">분석 기준일 ${esc(data.as_of)} · 코스피 ${esc(data.indices.find(i=>i.key==='KOSPI').source_session_date)} / 니케이 ${esc(data.indices.find(i=>i.key==='NIKKEI').source_session_date)} / 미국 ${esc(data.indices.find(i=>i.key==='NASDAQ').source_session_date)} · 지수 포인트를 선물 가격으로 환산하지 않음</div>
    ...
    <section class="chart-card cash-card"><h3>코스피 현물 · 지지 강도와 뷰 전환</h3><p class="cash-note">파랑은 지지, 빨강은 저항입니다. 5·10포인트 간격이 아니라 실제 고저점과 이평을 근거로 선택했어요. 코스피 ${number(data.indices.find(i=>i.key==='KOSPI').latest_daily_ohlc.close)}는 선물 가격과 구별합니다.</p>
